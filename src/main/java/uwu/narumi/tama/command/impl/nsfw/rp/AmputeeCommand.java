...
@CommandInfo(
        alias = "amputee",
        description = "?",
        usage = "/amputee <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class AmputeeCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://i.nhentai.net/galleries/1498830/14.png";

    public AmputeeCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s amputated some body parts of %s", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s amputated some body parts of %s", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
