...
@CommandInfo(
        alias = "drug",
        description = "?",
        usage = "/drug <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class DrugCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/f7/e5/f7e5c0765e1cdab9a1da844eaebe3f89.jpg";
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s drugged %s", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s drugged %s", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
