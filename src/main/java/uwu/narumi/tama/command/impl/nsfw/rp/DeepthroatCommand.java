...
@CommandInfo(
        alias = "deepthroat",
        description = "?",
        usage = "/deepthroat <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class DeepthroatCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/3e/9a/3e9ab0d91e9036227ab6e96774523ee8.gif";

    public DeepthroatCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s had deepthroat sex with %s", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s had deepthroat sex with %s", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
