...
@CommandInfo(
        alias = "lolibooru",
        description = "Shows random image from lolibooru",
        usage = "/lolibooru [tags] [rating] | /lolibooru $search",
        type = CommandType.NSFW,
        aliases = "loli"
)
public class LolibooruCommand extends Command implements BooruGrabber {

    private final String url = "https://lolibooru.moe/post/index.xml?" +
            "page=0" +
            "&limit=100" +
            "&tags=%s";

    public LolibooruCommand() {
        super(
    ...
    public MessageEmbed execute(Object... args) {
        ...
        if (elements.size() <= 0)
            throw new CommandException("No images found, with specified tags");
    ...
    public Optional<Image> fetchImage(int position, Elements elements) {
        ...
        return Optional.of(new Image(post.attr("file_url").replace(' ', '+'), post.attr("source").replace(' ', '+')));
    }
}