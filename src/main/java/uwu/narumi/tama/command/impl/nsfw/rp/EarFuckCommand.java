...
@CommandInfo(
        alias = "earfuck",
        description = "?",
        usage = "/earfuck <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class EarFuckCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//samples/9a/3d/sample_9a3d13fb804e05d1ad894f20f6de066f.jpg";

    public EarFuckCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s ear fucked %s", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s ear fucked %s", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
