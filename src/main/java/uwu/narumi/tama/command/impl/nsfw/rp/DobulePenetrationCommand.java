...
@CommandInfo(
        alias = "doublepen",
        description = "?",
        usage = "/doublepen <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class DobulePenetrationCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/88/15/88156c29cdcc3624f42f7b19cdb2df84.gif";

    public DobulePenetrationCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s was double penetrated by %s", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s was double penetrated by %s", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
