...
@CommandInfo(
        alias = "handjob",
        description = "?",
        usage = "/handjob <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class HandJobCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/b7/11/b7111ed99a43cc6fc054f599ada4fc2a.gif";

    public HandJobCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s gave %s handjob", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s gave %s handjob", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
