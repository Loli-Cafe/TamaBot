...
@CommandInfo(
        alias = "footjob",
        description = "?",
        usage = "/footjob <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class FootJobCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/48/78/48781d94488e2175ee6ce1d1d2530ed1.gif";

    public FootJobCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s gave footjob to %s", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s gave footjob to %s", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
