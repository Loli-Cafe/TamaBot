...
@CommandInfo(
        alias = "scat",
        description = "?",
        usage = "/scat [person (mention)]",
        type = CommandType.NSFW
)
public class ScatCommand extends Command implements NekosLifeGrabber {

    private static final String YOURSELF_URL = "https://wimg.rule34.xxx//images/4902/596f4b677bc8fe57f310302b335607bc.png?5581534";
    private static final String URL = "https://img.rule34.xxx//images/4930/185d61a1a1edd0e788816a1ece5b174f.gif?5611964";
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                (DiscordHelper.hasMention(event, Message.MentionType.USER)
                        ? String.format("%s shitted on %s", event.getAuthor(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
                        : (event.getAuthor().getAsMention() + " shit yourself")),
                DiscordHelper.hasMention(event, Message.MentionType.USER)
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                (optionMapping != null
                        ? String.format("%s shitted on %s", event.getUser(), optionMapping.getAsUser().getAsMention())
                        : (event.getUser().getAsMention() + " shit yourself")),
                optionMapping != null
        )).queue();
    }
    ...
    public MessageEmbed execute(Object... args) {
        return EmbedHelper.action((String) args[0], (boolean) args[1] ? URL : YOURSELF_URL);
    }
}
