...
@CommandInfo(
        alias = "whip",
        description = "?",
        usage = "/whip <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class WhipCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img3.gelbooru.com//images/7a/6f/7a6fa9a1ec6498e369f5066446e637f2.gif";
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s whipped %s", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s whipped %s", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
