...
@CommandInfo(
        alias = "gropeboobs",
        description = "?",
        usage = "/gropeboobs <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class GropeBoobsCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/cb/3b/cb3b94ad40da8411bcc4931087c20fc5.gif";

    public GropeBoobsCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s groped %s boobs", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s groped %s boobs", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
