...
@CommandInfo(
        alias = "blowjob",
        description = "?",
        usage = "/blowjob <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class BlowJobCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/c6/8d/c68d6325fcfcaa101755989d3333213c.gif";

    public BlowJobCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s gave blowjob to %s", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s give blowjob to %s", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
