...
@CommandInfo(
        alias = "creampie",
        description = "?",
        usage = "/creampie [person (mention)]",
        type = CommandType.NSFW
)
public class CreampieCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/ea/54/ea544fe28a11f760367963778b781e96.gif";

    public CreampieCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                DiscordHelper.hasMention(event, Message.MentionType.USER)
                        ? String.format("%s creampied %s", event.getAuthor(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
                        : (event.getAuthor().getAsMention() + " wants creampie")
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                optionMapping != null
                        ? String.format("%s creampied %s", event.getUser(), optionMapping.getAsUser().getAsMention())
                        : (event.getUser().getAsMention() + " creampie")
        )).queue();
    }
