...
@CommandInfo(
        alias = "piercelabia",
        description = "?",
        usage = "/piercelabia <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class PierceLabiaCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//samples/0d/dd/sample_0ddd8b34169a3c038f8f346095e90ca2.jpg";

    public PierceLabiaCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s pierced %s labia", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s pierced %s labia", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
