...
@CommandInfo(
        alias = "humiliate",
        description = "?",
        usage = "/humiliate <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class HumiliateCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//samples/50/8f/sample_508fb6fc36ea6590671afb78b6e0153e.jpg";

    public HumiliateCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s humiliates %s", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s humiliates %s", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
