...
public class ProlapsePussyCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//samples/e8/7c/sample_e87cd07dd12be5b75da1292aeaaae372.jpg";

    @Override
    public void compose(MessageReceivedEvent event, String... args) {
        checkContext(event.getMember(), event.getTextChannel());
        event.getTextChannel().sendMessageEmbeds(execute(String.format("%s showing her prolapsed pussy", event.getAuthor().getAsMention()))).queue();
    }

    @Override
    public void compose(SlashCommandInteractionEvent event) {
        checkContext(event.getMember(), event.getTextChannel());
        event.replyEmbeds(execute(String.format("%s showing her prolapsed pussy", event.getUser().getAsMention()))).queue();
    }
