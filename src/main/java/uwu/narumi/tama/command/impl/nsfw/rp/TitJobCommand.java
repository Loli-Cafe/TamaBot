...
@CommandInfo(
        alias = "titjob",
        description = "?",
        usage = "/titjob <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class TitJobCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img3.gelbooru.com//images/81/c1/81c1ad1f2dfc37e642fbfa3e56b6f32f.gif";

    public TitJobCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s gave titjob to %s", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s gave titjob to %s", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
