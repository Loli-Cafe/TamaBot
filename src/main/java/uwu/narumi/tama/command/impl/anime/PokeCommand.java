...
@CommandInfo(
        alias = "poke",
        description = "?",
        usage = "/poke <person (mention)>",
        type = CommandType.ANIME,
        argsLength = 1
)
public class PokeCommand extends Command implements NekosLifeGrabber {

    public PokeCommand() {
        super(
                new OptionData(OptionType.USER, "user", "Someone who you want to poke", true, false)
        );
    }
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s is poking %s", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s is poking %s", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
    ...
    public MessageEmbed execute(Object... args) {
        return EmbedHelper.action((String) args[0], fetchImage("poke").orElseThrow(() -> new CommandException("Something went wrong")));
    }
}
