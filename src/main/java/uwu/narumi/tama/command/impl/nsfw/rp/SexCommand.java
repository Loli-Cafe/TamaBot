...
@CommandInfo(
        alias = "sex",
        description = "?",
        usage = "/sex [person (mention)]",
        type = CommandType.NSFW
)
public class SexCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/8f/ae/8fae223ef7acb9272b785a69bd63c9ac.gif";
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                DiscordHelper.hasMention(event, Message.MentionType.USER)
                        ? String.format("%s and %s had sex", event.getAuthor(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
                        : (event.getAuthor().getAsMention() + " wants to have sex")
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                optionMapping != null
                        ? String.format("%s and %s had sex", event.getUser(), optionMapping.getAsUser().getAsMention())
                        : (event.getUser().getAsMention() + " wants to have sex")
        )).queue();
    }
