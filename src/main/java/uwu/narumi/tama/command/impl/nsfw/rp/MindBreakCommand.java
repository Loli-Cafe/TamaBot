...
@CommandInfo(
        alias = "mindbreak",
        description = "?",
        usage = "/mindbreak <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class MindBreakCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/5b/c5/5bc529bb14c35033b7a19103ba704afc.gif";

    public MindBreakCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s damaged %s mentality", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s damaged %s mentality", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
