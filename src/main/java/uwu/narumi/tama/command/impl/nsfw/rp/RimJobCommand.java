...
@CommandInfo(
        alias = "rimjob",
        description = "?",
        usage = "/rimjob [person (mention)]",
        type = CommandType.NSFW,
        argsLength = 1
)
public class RimJobCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/f3/2c/f32cf93275cda7c5ef69e0b90d4ac2b0.gif";

    public RimJobCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                DiscordHelper.hasMention(event, Message.MentionType.USER)
                        ? String.format("%s rimmed %s", event.getAuthor(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                optionMapping != null
                        ? String.format("%s rimmed %s", event.getUser(), optionMapping.getAsUser().getAsMention())
