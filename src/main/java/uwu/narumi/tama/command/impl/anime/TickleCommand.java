...
@CommandInfo(
        alias = "tickle",
        description = "?",
        usage = "/tickle <person (mention)>",
        type = CommandType.ANIME,
        argsLength = 1
)
public class TickleCommand extends Command implements NekosLifeGrabber {

    public TickleCommand() {
        super(
                new OptionData(OptionType.USER, "user", "Someone who you want to tickle", true, false)
        );
    }
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s tickles %s", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s tickles %s", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
    ...
    public MessageEmbed execute(Object... args) {
        return EmbedHelper.action((String) args[0], fetchImage("tickle").orElseThrow(() -> new CommandException("Something went wrong")));
    }
}
