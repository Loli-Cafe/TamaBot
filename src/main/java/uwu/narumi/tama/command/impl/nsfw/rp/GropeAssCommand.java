...
@CommandInfo(
        alias = "gropeass",
        description = "?",
        usage = "/gropeass <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class GropeAssCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/2a/55/2a55bf453d9e4a2d3e018d3da176e4de.gif";

    public GropeAssCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s groped %s ass", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s groped %s ass", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
