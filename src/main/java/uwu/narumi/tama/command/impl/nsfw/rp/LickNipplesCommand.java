...
@CommandInfo(
        alias = "licknipples",
        description = "?",
        usage = "/licknipples <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class LickNipplesCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/c8/09/c809bda7484bbf6e664dda095f7c7800.gif";

    public LickNipplesCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s licked %s nipples", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s licked %s nipples", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
