...
@CommandInfo(
        alias = "touchpussy",
        description = "?",
        usage = "/touchpussy <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class TouchPussyCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/4e/40/4e4039c691ac6f08e3c7b11c7f1ac987.gif";

    public TouchPussyCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s touched %s pussy", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s touched %s pussy", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
