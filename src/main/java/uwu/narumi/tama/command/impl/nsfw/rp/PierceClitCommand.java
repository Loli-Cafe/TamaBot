...
@CommandInfo(
        alias = "pierceclit",
        description = "?",
        usage = "/pierceclit <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class PierceClitCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/7f/81/7f815adf361f95c4aebca44cedd86190.jpeg";

    public PierceClitCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s pierced %s clit", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s pierced %s clit", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
