...
@CommandInfo(
        alias = "fistpussy",
        description = "?",
        usage = "/fistpussy <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class FistPussyCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img1.gelbooru.com//images/69/dc/69dc023805bd09474ffcc6b536361e1f.jpeg";

    public FistPussyCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s fisted %s pussy", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s fisted %s pussy", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
