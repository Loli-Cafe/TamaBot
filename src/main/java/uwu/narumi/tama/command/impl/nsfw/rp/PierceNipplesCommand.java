...
@CommandInfo(
        alias = "piercenipples",
        description = "?",
        usage = "/piercenipples <person (mention)>",
        type = CommandType.NSFW,
        argsLength = 1
)
public class PierceNipplesCommand extends Command implements NekosLifeGrabber {

    private static final String URL = "https://img3.gelbooru.com//images/88/f6/88f61c2efe688092baeebc6864ad95f1.jpg";

    public PierceNipplesCommand() {
        super(
    ...
    public void compose(MessageReceivedEvent event, String... args) {
        ...
        event.getTextChannel().sendMessageEmbeds(execute(
                String.format("%s pierced %s nipples", event.getAuthor().getAsMention(), DiscordHelper.getMention(event, Message.MentionType.USER).getAsMention())
        )).queue();
    }
    ...
    public void compose(SlashCommandInteractionEvent event) {
        ...
        event.replyEmbeds(execute(
                String.format("%s pierced %s nipples", event.getUser().getAsMention(), optionMapping.getAsUser().getAsMention())
        )).queue();
    }
